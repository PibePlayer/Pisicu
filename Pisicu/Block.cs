...
namespace Pisicu{
    ...
    public class Block {
        ...
        public Block setStroke(int stroke,Color stroke_color) {
        ...
        public void draw(SpriteBatch sb, bool solid,float x, float y, float w, float h, float rad, Color color){
            ...
            }if(stroke != 0) { //----> CORNER BLOCK 

                int r = (int)((13 * rad / 190) + stroke) - 1;

                if(stroke_color == ColorBank.undefined) {stroke_color = color;}

                sb.Draw(Game1.point, new Rectangle((int)(x + rad), (int)y, (int)(w - 2 * rad), r), stroke_color); //SOLID CENTER UP
                sb.Draw(Game1.point, new Rectangle((int)(x + rad), (int) (y + h - r), (int)(w - 2 * rad), r), stroke_color); //SOLID CENTER DOWN

                sb.Draw(Game1.point, new Rectangle((int)x, (int)(y + rad), r, (int)(h - 2 * rad)), stroke_color); //SOLID LEFT
                sb.Draw(Game1.point, new Rectangle((int)(x + w - r), (int)(y + rad), r, (int)(h - 2 * rad)), stroke_color); //SOLID RIGHT
